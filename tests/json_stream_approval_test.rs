...
async fn test_tool_approval_approve_flow() {
    ...
    let result = engine.run("Use the tool", "msg-1", None).await.expect("should succeed");
...
async fn test_tool_approval_deny_flow() {
    ...
    let result = engine.run("Use the tool", "msg-2", None).await.expect("should succeed");
...
async fn test_auto_approve_bypasses_approval() {
    ...
    let result = engine.run("Use the tool", "msg-3", None).await.expect("should succeed");
...
async fn test_session_auto_approve_category() {
    ...
    // No background task to approve — should not hang
    let result = engine.run("Use the tool", "msg-4", None).await.expect("should succeed");
...
async fn test_client_disconnect_aborts() {
    ...
    let err = engine.run("Use the tool", "msg-5", None).await.unwrap_err();
    assert!(

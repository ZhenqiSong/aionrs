...
async fn case_9_off_vs_safe_content() {
    ...
    let result = engine.run(prompt, "", None).await.expect("engine.run should succeed");
...
async fn case_10_off_vs_full_token_savings() {
    ...
    let result_off = engine_off.run(prompt, "", None).await.expect("engine.run should succeed");
    ...
    let result_full = engine_full.run(prompt, "", None).await.expect("engine.run should succeed");
